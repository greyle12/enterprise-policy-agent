from scripts.verify_postgres_submission_repository import run_verification


def test_offline_postgres_submission_repository_verification_passes() -> None:
    result = run_verification()

    assert result["phase"] == 38
    assert result["step"] == 3
    assert result["substep"] == 3
    assert result["status"] == "repository_only"
    assert result["passed"] is True
    assert result["postgres_schema_version"] == 1
    assert result["repository"] == "PostgresMockApprovalSubmitter"
    assert result["domains"] == ["approval_submission", "submission_audit"]
