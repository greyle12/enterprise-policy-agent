...
def test_builds_classification_messages_with_trimmed_input() -> None:
    llm_client = FakeLLMClient(
        '{"intent":"policy_query","confidence":0.98,'
        '"reason":"查询制度"}'
    )
    classifier = IntentClassifier(
        llm_client=llm_client,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            '{"intent":"policy_query","confidence":1.1,'
            '"reason":"置信度越界"}'
        ),
        (
            '{"intent":"policy_query","confidence":"0.9",'
...
def test_falls_back_to_unknown_below_confidence_threshold() -> None:
    classifier = IntentClassifier(
        llm_client=FakeLLMClient(
            '{"intent":"approval_query","confidence":0.59,'
            '"reason":"可能在询问审批"}'
        ),
    )

    result = asyncio.run(
        classifier.classify("这个要怎么处理？")
    )
