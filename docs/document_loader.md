...
因此本阶段不会把权限过滤移动到检索之后。Advanced RAG Phase 29 接入 pgvector 后，允许范围
已经转换成参数化 SQL 条件，并先物化授权候选，再执行相似度排序。Loader 产出的文本也不能被
视为可信指令；召回后仍会经过证据污染检测。
...
`allowed_record_ids` 同时交给 Vector 与 BM25。RRF 只能融合两路已经授权的排名，Phase 28 Reranker
也只能接收这批授权候选。
