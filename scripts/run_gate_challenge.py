...
CHALLENGE = ROOT / "artifacts/coverage-gate-challenge-v1"


def load_challenge():
    lock = json.loads((CHALLENGE / "challenge-lock.json").read_text(encoding="utf-8"))
    freeze = json.loads((CHALLENGE / "gate-freeze.json").read_text(encoding="utf-8"))
    require(
        digest((CHALLENGE / "gate-freeze.json").read_bytes()) == lock["gate_freeze_sha256"],
        "freeze changed",
    )
...
def score_selection(case, base, pool, allowed, links, window):
    ...
    gated, reason = select_gated(case["query"], base, frozen, events, "explicit_veto")
    elapsed = (perf_counter() - start) * 1000
    judgments = [{"chunk_id": j["chunk_id"], "relevance": j["grade"]} for j in case["judgments"]]
    ids = {"raw": base, "frozen": frozen, "gated": gated}
    ...
    return {
        ...
        "blocked": frozen != gated,
        "evicted_ids": [i for i in base if i not in gated],
        "regressions": [
            k for k in metrics["gated"] if metrics["gated"][k] < metrics["frozen"][k] - 1e-12
        ],
...
def run(args):
    ...
    result = {
        ...
        "timing_scope": "selection only; excludes model inference; single sample",
        "windows": {},
    }
    for window in (20, 40):
    ...
    write(args.output_dir / "challenge-report.json", result)
