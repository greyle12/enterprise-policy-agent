...
Platform Phase 38：Multi-instance Shared State（进行中：Step 3.1 Session/Draft Repository 已完成）
...
- [ ] Phase 38 PostgreSQL Agent shared state 与 Redis session coordination（Step 3.1 已完成，Repository 其余领域与运行时迁移尚未开始）；
...
- [ ] Step 3.2–3.4 Conversation、Submission/Audit Repository 与真实 PostgreSQL 集成验收；
...
当前已有 Step 3.1 的 Session/Draft Repository，但 FastAPI 默认和 Compose runtime 仍使用 SQLite，且**没有**复制
SQLite 数据、获取 Redis session lock、接入 PostgreSQL Repository 或切换 persistence backend。完整设计和状态归属见
