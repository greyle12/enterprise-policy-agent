...
Platform Phase 38：Multi-instance Shared State（进行中：Step 3.4 真实 PostgreSQL 验收 Gate 已就绪）
...
- [ ] Phase 38 PostgreSQL Agent shared state 与 Redis session coordination（Step 3.4 真实数据库 Gate 已就绪，等待 PostgreSQL/CI 实跑通过；运行时迁移尚未开始）；
...
- [x] Step 3.1 async PostgreSQL connection boundary 与 Session/Draft Repository；
- [x] Step 3.2 PostgreSQL Conversation Repository；
- [x] Step 3.3 PostgreSQL Submission/Audit Repository；
- [ ] Step 3.4 真实 PostgreSQL Repository 集成与并发验收（Gate 已就绪；需真实 PostgreSQL/CI 六项通过）；
...
当前已有 Step 3.1–3.3 的 Session/Draft/Conversation/Submission/Audit Repository，以及 Step 3.4 的隔离测试库、
Windows-compatible SelectorEventLoop、并发集成测试和 CI Gate；但 FastAPI 默认和 Compose runtime 仍使用
SQLite，且**没有**复制 SQLite 数据、获取 Redis session lock、接入 PostgreSQL Repository 或切换 persistence
backend。完整设计和状态归属见
