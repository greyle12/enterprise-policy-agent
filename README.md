...
Platform Phase 38：Multi-instance Shared State（进行中：Step 3.3 Submission/Audit Repository 已完成）
...
- [ ] Phase 38 PostgreSQL Agent shared state 与 Redis session coordination（Step 3.3 已完成，真实 PostgreSQL 集成验收与运行时迁移尚未开始）；
...
- [ ] Step 3.4 真实 PostgreSQL Repository 集成与并发验收；
...
当前已有 Step 3.1–3.3 的 Session/Draft/Conversation/Submission/Audit Repository，但 FastAPI 默认和 Compose
runtime 仍使用 SQLite，且**没有**复制 SQLite 数据、获取 Redis session lock、接入 PostgreSQL Repository 或
切换 persistence backend。完整设计和状态归属见
